...
import { BarChart3, PieChart as PieIcon, MapPin, Landmark, Target } from 'lucide-react';

export default function AdminInsights({ ideas = [], matches = [], users = [] }: any) {
  
  // 1. STATE SAKTI ANTI-ERROR
  const [isMounted, setIsMounted] = useState(false);

  useEffect(() => {
    // Memberi tahu React bahwa komponen sudah di-mount di browser
    // dan aman untuk merender grafik Recharts
    setIsMounted(true);
  }, []);
  ...
  return (
    <div className="space-y-10">
      ...
      {/* Jika belum mounted, jangan render Recharts dulu (cegah error -1) */}
      {!isMounted ? (
        <div className="h-[450px] flex items-center justify-center text-slate-300 font-bold uppercase text-[10px] animate-pulse">
          Loading Visuals...
        </div>
      ) : (
        <>
...
function ChartContainer({ title, icon, children }: any) {
  return (
    // 1. Hapus fix h-[450px] dari sini, biarkan tingginya menyesuaikan isi
    <div className="bg-white p-10 rounded-[50px] border border-slate-100 flex flex-col">
      <h4 className="text-[11px] font-black uppercase text-slate-900 mb-8 tracking-widest flex items-center gap-2">
        {icon} {title}
      </h4>
      {/* 2. OBAT FINAL: Beri tinggi pasti di sini (misal: h-[300px]). Hilangkan flex-1 */}
